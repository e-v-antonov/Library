...
namespace Library
{
    public partial class ReturnBookForm : Form
    {
        ...
        private void btnReturnBook_Click(object sender, EventArgs e)    //клик по кнопке возврата книги
        {
            ...
            commandReader.CommandText = "select count(*) from [dbo].[Registration_Card_Reader_View] where [Surname_Reader] = '" + tbSurname.Text +
                "' and [Name_Reader] = '" + tbName.Text + "' and [Patronymic_Reader] = '" + tbPatronymic.Text + "' and [Passport_Series_Reader] = '" + 
                tbPassportSeries.Text + "' and [Passport_Number_Reader] = '" + tbPassportNumber.Text + "'"; //кол-во людей с таким ФИО
            ...
            if (countReader == 0)   //если такого читателя нет
            {
            ...
            else     //если читатель есть
            {
                try
                {
                    SqlCommand commandIDReader = new SqlCommand("", RegistryData.DBConnectionString);
                    commandIDReader.CommandText = "select [ID_Registration_Card_Reader] from [dbo].[Registration_Card_Reader_View] where " +
                        "[Surname_Reader] = '" + tbSurname.Text + "' and [Name_Reader] = '" + tbName.Text + "' and [Patronymic_Reader] = '" + 
                        tbPatronymic.Text + "' and [Passport_Series_Reader] = '" + tbPassportSeries.Text + "' and [Passport_Number_Reader] = '" + 
                        tbPassportNumber.Text + "'";    //узнаем ID читателя
                    RegistryData.DBConnectionString.Open();
                    DBTables dbTables = new DBTables();
                    dbTables.CommandOpenKey.ExecuteNonQuery();
                    ...
                    commandIDFormular.CommandText = "select [ID_Formular_Reader] from [dbo].[Formular_Reader] where [Book_Returned] = 0 and " +
                        "[Registration_Card_Reader_ID] = " + idReader.ToString();   //узнаем ID формуляра читателя
                ...
                if (idFormular > 0)
                {
                    ...
                    MessageBox.Show(MessageUser.BookReturnedSucc, MessageUser.TitleLibrary, MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
        }
