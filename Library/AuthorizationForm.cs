    ...
    public partial class AuthorizationForm : Form
    {
        ...
        private void btnAuthorization_Click(object sender, EventArgs e) //клик по кнопке авторизация
        {
            ...
            else
            {
                ...
                SqlCommand commandUserID = new SqlCommand("", RegistryData.DBConnectionString);
                ...
                commandUserID.CommandText = "select [ID_User] from [dbo].[User] where CONVERT([nvarchar] (16), DECRYPTBYKEY([Login_User])) = '" + tbLogin.Text + "'";
                ...
                else     //установление роли данного пользователя
                {
                    ...
                    UserID = Convert.ToInt32(commandUserID.ExecuteScalar().ToString());
                                       
                    dbTables.CommandCloseKey.ExecuteNonQuery();
                    RegistryData.DBConnectionString.Close();
                                        
                    if (userRole == 3)  // если вошел директор
                    {
                        RegistryData.DBConnectionString.Open();
